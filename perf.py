...
def plot_results(n_values, times, project):
    ...
    n3_values = [n**3 for n in n_values]

    # Plot
    ax1.plot(n3_values, times, "bo-", linewidth=2, markersize=8)
    ...
    for n, t in zip(n3_values, times):
        ax1.annotate(
            f"{t:.3f}s",
            (n, t),
    ...
    # Table
    table_data = [[n, n2, f"{t:.4f}"] for n, n2, t in zip(n_values, n3_values, times)]
...
if __name__ == "__main__":
    ...
    for i in range(len(n_values)):
        print(f"N={n_values[i]}, N^3={n_values[i] ** 3}, tempo={times[i]}")
    print("[")
    for i in range(len(n_values)):
