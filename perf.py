...
def run_tests(project, pmax, n_start, N_max, repetitions, seed=None):
    ...
    for i in range(repetitions):
        ...
        try:
            ...
            print(f"ERROR: {e}")
            break
        current_n = n_start + round((((N_max**3)-(n_start**3))*(i+1) / (repetitions-1))**(1/3))
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        print("Usage: python3 perf.py <project> <Pmax> <N_start> <N_max> <repetitions> [seed]")
